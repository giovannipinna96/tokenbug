...
- Batch size 16 (possible with LoRA)
...
def main():
    ...
    OUTPUT_DIR = Path('./models/finetuned-nomic-embed')
    ...
    # Training hyperparameters (optimized for A100 80GB with LoRA)
    BATCH_SIZE = 32  # Increased - LoRA uses much less memory
    NUM_EPOCHS = 3
    WARMUP_STEPS = 1000  # Increased for larger dataset
    LEARNING_RATE = 3e-4  # Higher LR for LoRA (recommended: 1e-4 to 5e-4)
    EVALUATION_STEPS = 2000  # Increased for larger dataset
    ...
    print("=" * 80)
    print("Fine-tuning nomic-embed-code for Bug Detection")
    print("=" * 80)
    print(f"\nConfiguration:")
